    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        {stats.map(s => (
          <div key={s.label} className="bg-slate-50 rounded-xl px-4 py-3">
            ...
            <p className="text-lg font-bold text-slate-900 tabular-nums mt-0.5">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-2.5 text-sm">
        <div className="flex items-start gap-3 py-2.5 border-b border-slate-100">
          <span className="text-slate-400 w-28 shrink-0">Company</span>
        ...
        <div className="flex items-start gap-3 py-2.5 border-b border-slate-100">
          <span className="text-slate-400 w-28 shrink-0">Confidence</span>
          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${CONFIDENCE_COLORS[bot.confidence]}`}>
            {bot.confidence}
          </span>
        </div>
        <div className="flex items-start gap-3 py-2.5 border-b border-slate-100">
          <span className="text-slate-400 w-28 shrink-0 mt-0.5">User-Agent</span>
          <span className="font-mono text-xs text-slate-600 break-all">{bot.userAgent}</span>
        </div>
        <div className="flex items-start gap-3 py-2.5 border-b border-slate-100">
          <span className="text-slate-400 w-28 shrink-0">First Seen</span>
          <span className="text-slate-700">{firstSeenDate(bot.daysAgoFirst).toLocaleDateString('en-US', { year:'numeric', month:'short', day:'numeric' })}</span>
        </div>
        <div className="flex items-start gap-3 py-2.5 border-b border-slate-100">
          <span className="text-slate-400 w-28 shrink-0">Last Seen</span>
          <span className="text-slate-700">{timeAgo(lastSeenDate(bot.daysAgoSeen))}</span>
        </div>
        {bot.docUrl && (
          <div className="flex items-start gap-3 py-2.5">
            <span className="text-slate-400 w-28 shrink-0">Docs</span>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-slate-50 rounded-xl p-4 space-y-3">
        <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Access Status</p>
          ...
          {(['allowed','blocked','pending'] as BotStatus[]).map(s => {
            ...
            return (
              <button key={s} onClick={() => setStatusLocal(s)}
                className={`flex-1 py-2 rounded-lg border text-sm font-semibold capitalize transition-all ${status === s ? colors : 'border-slate-200 text-slate-500 hover:bg-slate-100'}`}>
                {s}
              </button>
            );
          })}
      ...
      <div className="space-y-3">
        <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Access Level</p>
        {[
          ...
          <label key={opt.val}
            className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all ${accessLevel === opt.val ? 'border-brand-dark bg-brand-dark/5' : 'border-slate-200 hover:border-slate-300'}`}>
      ...
      {accessLevel === 'custom' && (
        <div className="space-y-3 bg-slate-50 rounded-xl p-4">
          <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Custom Pricing</p>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-xs text-slate-500">Price/Page (USDC)</label>
              <div className="relative mt-1">
                <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs">$</span>
                <input type="number" min="0" step="0.0001" value={ppp}
                  onChange={e => setPpp(parseFloat(e.target.value)||0)}
                  className="w-full pl-5 pr-2 py-2 rounded-lg border border-slate-200 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark" />
              </div>
            </div>
            <div>
              <label className="text-xs text-slate-500">Free Tier Limit/day</label>
              <input type="number" min="0" value={freeLimit}
                onChange={e => setFreeLimit(parseInt(e.target.value)||0)}
                className="mt-1 w-full px-2 py-2 rounded-lg border border-slate-200 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark" />
            </div>
            <div>
              <label className="text-xs text-slate-500">Fee % (Platform)</label>
              <div className="relative mt-1">
                <input type="number" min="0" max="100" value={feePct}
                  onChange={e => setFeePct(parseInt(e.target.value)||0)}
                  className="w-full pr-5 px-2 py-2 rounded-lg border border-slate-200 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark" />
    ...
    <div className="space-y-5">
      ...
      <label className="flex items-center justify-between p-4 bg-slate-50 rounded-xl cursor-pointer">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Enforce request limits for this bot</p>
        </div>
      ...
      <div className={`space-y-3 transition-opacity ${enabled ? '' : 'opacity-40 pointer-events-none'}`}>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Requests / Hour</label>
            <input type="number" min="1" value={perHour} onChange={e => setPerHour(parseInt(e.target.value)||1)}
              className="mt-1.5 w-full px-3 py-2.5 rounded-lg border border-slate-200 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Requests / Day</label>
        ...
        <div>
          <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">When Limit Exceeded</p>
          <div className="space-y-2">
            {[
              ...
              <label key={opt.val}
                className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all ${action === opt.val ? 'border-brand-dark bg-brand-dark/5' : 'border-slate-200 hover:border-slate-300'}`}>
                <input type="radio" className="mt-0.5 accent-brand-dark shrink-0"
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-0 divide-y divide-slate-50 rounded-xl border border-slate-100 overflow-hidden">
        {events.map(e => (
          <div key={e.id} className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors text-sm">
            <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${e.status === 'paid' ? 'bg-emerald-500' : 'bg-slate-300'}`} />
            <span className="font-mono text-xs text-slate-600 flex-1 truncate">{e.path}</span>
            <span className={`text-xs font-semibold ${e.status === 'paid' ? 'text-emerald-600' : 'text-slate-400'}`}>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          <div className="flex items-start gap-3">
            {/* Bot initial avatar */}
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm shrink-0 ${TYPE_COLORS[bot.type] ?? 'bg-slate-100 text-slate-600'}`}>
              {bot.name[0]}
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="font-bold text-slate-900 text-base">{bot.name}</h2>
              ...
              <p className="text-xs text-slate-500 mt-0.5">{bot.company} · {TYPE_LABELS[bot.type]}</p>
            </div>
          </div>
        ...
        {/* Tab bar */}
        <div className="flex border-b border-slate-100 px-6 shrink-0 bg-slate-50">
          {TABS.map(t => {
            const Icon = t.icon;
            return (
