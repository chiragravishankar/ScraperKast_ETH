    ...
    <div className="space-y-4">
      <div>
        ...
        <input type="text" value={draft.name} onChange={e => set({ name: e.target.value })}
          ...
          className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark" />
      </div>
      <div>
        ...
        <textarea value={draft.description} onChange={e => set({ description: e.target.value })}
          ...
          rows={2}
          className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark resize-none" />
      </div>
      <label className="flex items-center justify-between p-3.5 bg-slate-50 rounded-xl cursor-pointer border border-slate-200">
        <div>
          <p className="text-sm font-semibold text-slate-800">Enabled</p>
          <p className="text-xs text-slate-500 mt-0.5">Disabled rules are skipped during matching</p>
        </div>
...
function StepPathPattern({ draft, set }: { draft: Draft; set: (p: Partial<Draft>) => void }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Match Type</p>
        <div className="grid grid-cols-2 gap-2">
          {types.map(t => (
            <label key={t.val}
              className={`flex items-start gap-2.5 p-3 rounded-xl border cursor-pointer transition-all ${draft.pathMatch.type === t.val ? 'border-brand-dark bg-brand-dark/5' : 'border-slate-200 hover:border-slate-300'}`}>
      ...
      {draft.pathMatch.type !== 'all' && (
        <div>
          ...
          <input type="text" value={draft.pathMatch.pattern}
            ...
            className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark" />
        </div>
      )}
      ...
      {examples.length > 0 && (
        <div className="bg-slate-50 rounded-xl px-4 py-3">
          ...
          {examples.map(e => (
            <p key={e} className="text-xs font-mono text-slate-600 py-0.5">{e}</p>
          ))}
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        {types.map(t => (
          <label key={t.val}
            className={`flex flex-col gap-1 p-3 rounded-xl border cursor-pointer transition-all ${draft.botMatch.type === t.val ? 'border-brand-dark bg-brand-dark/5' : 'border-slate-200 hover:border-slate-300'}`}>
            ...
            <p className="text-xs text-slate-500 ml-5">{t.hint}</p>
          </label>
        ))}
      </div>
      ...
      {draft.botMatch.type === 'group' && (
        <div>
          <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Select Groups</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {BOT_GROUPS.map(g => (
              <label key={g.id}
                className={`flex items-center gap-2 p-3 rounded-xl border cursor-pointer transition-all ${draft.botMatch.groupIds.includes(g.id) ? 'border-brand-dark bg-brand-dark/5' : 'border-slate-200 hover:border-slate-300'}`}>
      ...
      {draft.botMatch.type === 'specific' && (
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
            {ALL_BOTS.map(b => (
              <label key={b.id}
                className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer text-sm transition-all ${draft.botMatch.botIds.includes(b.id) ? 'border-brand-dark bg-brand-dark/5 font-medium text-brand-dark' : 'border-slate-200 text-slate-600 hover:border-slate-300'}`}>
...
function StepPricingAdvanced({ draft, set }: { draft: Draft; set: (p: Partial<Draft>) => void }) {
  ...
  return (
    <div className="space-y-4">
      {/* Price */}
      <div>
        <label className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Price Per Page (µUSDC)</label>
        <div className="mt-1.5 flex gap-3 items-center">
          <input type="number" min="0" value={draft.pricePerPage}
            ...
            className="flex-1 px-3 py-2.5 rounded-xl border border-slate-200 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark" />
        ...
        <div className="flex gap-1.5 mt-2 flex-wrap">
          {[0, 500, 1000, 2000, 5000].map(p => (
            <button key={p} onClick={() => set({ pricePerPage: p })}
              className={`text-xs px-2.5 py-1 rounded-lg border transition-colors ${draft.pricePerPage === p ? 'bg-brand-dark text-white border-brand-dark' : 'border-slate-200 text-slate-500 hover:border-brand-dark hover:text-brand-dark'}`}>
              {p === 0 ? 'Free' : `${p} µ`}
            </button>
          ))}
        </div>
      ...
      <div>
        ...
        <div className="mt-1.5 grid grid-cols-2 gap-2">
          {licenses.map(l => (
            <label key={l.val}
              className={`flex items-center gap-2 p-3 rounded-xl border cursor-pointer transition-all ${draft.licenseType === l.val ? 'border-brand-dark bg-brand-dark/5' : 'border-slate-200 hover:border-slate-300'}`}>
              <input type="radio" className="accent-brand-dark shrink-0"
        ...
        {draft.licenseType === 'custom' && (
          <input type="text" value={draft.customLicense}
            ...
            className="mt-2 w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark" />
        )}
      </div>
      ...
      <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 space-y-3">
        <label className="flex items-center justify-between cursor-pointer">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Allow N free requests per day before charging</p>
          </div>
        ...
        {draft.freeTierOverride.enabled && (
          <div className="flex items-center gap-2">
            <input type="number" min="0" value={draft.freeTierOverride.limit}
              ...
              className="w-24 px-3 py-2 rounded-lg border border-slate-200 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark" />
            <span className="text-sm text-slate-500">free requests per day</span>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        <textarea value={draft.notes} onChange={e => set({ notes: e.target.value })}
          ...
          className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark" />
      </div>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          <h2 className="font-bold text-slate-900">{isEdit ? 'Edit Rule' : 'New Pricing Rule'}</h2>
        ...
        {/* Step indicator */}
        <div className="flex px-6 py-3 border-b border-slate-100 gap-1 shrink-0">
          {STEPS.map((s, i) => (
            <button key={s} onClick={() => setStep(i)}
              className={`flex-1 text-center text-xs font-medium py-1.5 rounded-lg transition-colors ${i === step ? 'bg-brand-dark text-white' : i < step ? 'bg-emerald-100 text-emerald-700' : 'text-slate-400 hover:text-slate-600'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <p className="text-sm font-semibold text-slate-700 mb-4">{STEPS[step]}</p>
