    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-ink-2 mt-0.5">Your ScraperKast platform wallet</p>
        </div>
        <button
          onClick={() => { void refreshBalance(); }}
          disabled={isLoading}
          className="flex items-center gap-1.5 text-xs text-ink-3 hover:text-brand-dark transition-colors"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        {/* Quick actions */}
        <div className="bg-white rounded-2xl border border-edge p-5 flex flex-col gap-3">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 border border-edge text-ink rounded-xl font-semibold hover:bg-canvas transition-colors text-sm"
          >
          ...
          <Link
            ...
            className="w-full flex items-center gap-3 px-4 py-3 border border-edge text-ink-2 rounded-xl font-semibold hover:bg-canvas transition-colors text-sm"
          >
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-sm font-semibold text-ink">Auto-Withdraw</h2>
          ...
          <button
            ...
            className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${awForm.enabled ? 'bg-brand-dark' : 'bg-edge'}`}
          >
        ...
        {awForm.enabled && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-edge-2">
            <div>
              ...
              <div className="mt-1.5 relative">
                ...
                <input
                  ...
                  className="w-full pl-7 pr-4 py-2.5 rounded-lg border border-edge focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark text-ink font-mono"
                />
              </div>
            </div>
            <div>
              ...
              <input
                ...
                className="mt-1.5 w-full px-3 py-2.5 rounded-lg border border-edge focus:outline-none focus:ring-2 focus:ring-brand-dark/30 focus:border-brand-dark text-ink font-mono text-sm"
              />
            </div>
          </div>
        )}
      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-4 border-b border-edge-2 flex items-center justify-between">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-edge-2 text-left">
                <th className="px-4 py-3 font-medium text-ink-3 text-xs uppercase tracking-wide">Type</th>
                <th className="px-4 py-3 font-medium text-ink-3 text-xs uppercase tracking-wide text-right">Amount</th>
                <th className="px-4 py-3 font-medium text-ink-3 text-xs uppercase tracking-wide">Status</th>
                <th className="px-4 py-3 font-medium text-ink-3 text-xs uppercase tracking-wide">When</th>
              ...
              {history.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center">
                    <Zap className="w-6 h-6 text-edge mx-auto mb-2" />
                ...
                history.slice(0, 10).map(item => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-canvas transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-xs text-ink-3 whitespace-nowrap">
      ...
      {process.env.NODE_ENV === 'development' && (
        <details className="bg-slate-900 rounded-xl text-xs font-mono text-slate-300 overflow-hidden">
          <summary className="px-4 py-2.5 cursor-pointer text-ink-3 hover:text-slate-200 select-none">
            🔍 Debug Info
          </summary>
          <div className="px-4 pb-4 pt-3 space-y-1 border-t border-slate-700">
            <div><span className="text-ink-2">smart wallet :</span> {smartWalletAddress}</div>
            <div><span className="text-ink-2">balance (µ)  :</span> {balance}</div>
            <div><span className="text-ink-2">balance USDC :</span> {(balance / 1_000_000).toFixed(6)}</div>
            <div><span className="text-ink-2">network      :</span> {network}</div>
            <button
              onClick={() => { void refreshBalance(); }}
